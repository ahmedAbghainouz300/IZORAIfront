...
export default function Marchandise() {
  ...
  const fetchAllMarchandises = () => {
    marchandiseService.getAll()
      ...
      .catch(error => {
        ...
        enqueueSnackbar("Erreur lors du chargement des marchandises", { variant: "error" });
      });
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm("Êtes-vous sûr de vouloir supprimer cette marchandise ?")) {
      marchandiseService.delete(id)
        .then(() => {
          enqueueSnackbar("Marchandise supprimée avec succès", { variant: "success" });
          fetchAllMarchandises();
        })
        .catch(error => {
  ...
  const columns = [
    ...
    { 
      ...
      flex: 1,
      valueGetter: (params) =>{ return params ? params.libelle : "N/A"}
    },
